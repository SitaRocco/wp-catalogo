...
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
   <meta name="description" content="Descubre lo mejor del Britpop: álbums icónicos y el legado de este movimiento musical británico de los años 90.">
...
<script>
    ...
    const cargarImagen = (entradas, observador) => {
        entradas.forEach((entrada) => {
            console.log(entrada.target.id, entrada.isIntersecting);
...
function addBlur(image) {
    ...
    paragraph.innerText = band;
    image.parentNode.appendChild(paragraph);
}


function removeBlur(image) {
    image.style.filter = "none";
    const paragraph = image.parentNode.querySelector(".image-description");
    if (paragraph) {
        paragraph.remove();
    }
